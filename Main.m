...
% Move the feeder robot to a new position
feederRobot.model.base = transl(1, 0, 0);  % Move feeder robot to (X=1, Y=-0.5)
...
welderRobot.model.base = transl(-1, 0, 0);  % Move welder robot to (X=-1, Y=0.5)
% Plot both robots in the environment
feederRobot.model.plot(zeros(1, feederRobot.model.n), 'workspace', [-2.5 2.5 -2.5 2.5 0 2.5]);
welderRobot.model.plot(zeros(1, welderRobot.model.n), 'workspace', [-2.5 2.5 -2.5 2.5 0 2.5]);
...
% addlistenexiter(app.Link2, 'ValueChanged', @(src, event) updateJoint(2, app, feederRobotWrapper.robot));
