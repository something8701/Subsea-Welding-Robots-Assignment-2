classdef UFOFleet < handle
    %%UFOFleet A way of creating a group of UFOs
    ...
    methods
        ...
        function self = UFOFleet(count)
            ...
            for i = 1:self.count
                ...
                self.model{i}.base = self.model{i}.base.T * transl(0,0,2 + rand * (self.flightVolume(3)-2));
        ...
        function PlotSingleRandomStep(self)
            for index = 1:self.count
                ...
                % Move Up (or down)
                self.model{index}.base = self.model{index}.base.T *  transl(0,0, (rand - 0.5) * 0.2);
                animate(self.model{index},0);
                
                % Turn randomly
                % Save base as a temp variable
                tempBase = self.model{index}.base.T;
                ...
                % Get base as temp
                tempBase = self.model{index}.base.T;
                if tempBase(3,4) < self.workspaceDimensions(5) + 1
                    self.model{index}.base = tempBase * transl(0,0,0.2);
                end
