...
function teach(robot, varargin)
    ...
    %---- handle options
    opt.deg = true;
    % UPDATE: Looking at differences between orientation calculation in V9
    % and V10 toolboxes and options 'rpy', 'rpy/xyz' and 'rpy/zyx'.
    opt.orientation = {'rpy', 'rpy/xyz', 'rpy/zyx', 'eul', 'approach'};
    opt.d_2d = false;
    opt.callback = [];
    opt.record = [];   
    ...
    if isempty(rhandles)
        ...
        if isempty(q) && ~isempty(info.q)
            ...
            q = info.q;
        elseif isempty(find(q ~= info.q,1))
            % Nothing to do since the graphical model is equal the given
            % joint state 
        else
            % joint coordinates were given, make them current
            disp('Updating plot or adding a plot. To avoid this, pass the current joint state when calling teach');
            robot.plot(q, args{:});
        end
    end
