...
function startup_rtb(tbpath)
	...
	addpath( fullfile(tbpath, 'ply') ); % Added By UTS
	addpath( fullfile(tbpath, 'UTS/Foundation') ); % Added By UTS
	addpath( fullfile(tbpath, 'UTS/RobotModels') );% Added By UTS
	addpath( fullfile(tbpath, 'UTS/Parts') );% Added By UTS
	addpath( fullfile(tbpath, '/data/panda/robots') );% Added By UTS
    ...
    if exist(p, 'dir')
        warning off % Some computers get a warning about "Unable to obtain a change notification handle"
        addpath( genpath(p) );
        warning on
