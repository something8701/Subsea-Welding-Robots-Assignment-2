classdef Underwater_Welding_Environment_V2 < handle
    properties
    ...
    methods
        ...
        function self = Underwater_Welding_Environment_V2(NX, NY, NZ)
            if nargin < 1
                NX = 5; % Default size
            end
            if nargin < 2
        ...
        function plotEnvironment(self, hAxes)
            ...
            % Plot the ceiling
                ...
                surface('XData', X_wall, 'YData', Y_wall, 'ZData', Z_ceil, ...
                    ...
                    'EdgeColor', 'none', 'Parent', hAxes);
            % Place ROV
                    ROV = PlaceObject('ROV_V2.PLY',[0,-0.2,0]);
                    verts = [get(ROV,'Vertices'), ones(size(get(ROV,'Vertices'),1),1)];
                    set(ROV,'Vertices',verts(:,1:3))
            % Place Stop Button
                Stop_1 = PlaceObject('StopButton.ply',[0.3,-0.5,0.5]);
                verts = [get(Stop_1,'Vertices'), ones(size(get(Stop_1,'Vertices'),1),1)];
                set(Stop_1,'Vertices',verts(:,1:3))
            % Place Stop Button
                Stop_2 = PlaceObject('StopButton.ply',[-2.5,-2.5,0]);
                verts = [get(Stop_2,'Vertices'), ones(size(get(Stop_2,'Vertices'),1),1)];
                set(Stop_2,'Vertices',verts(:,1:3))
